...
public sealed class MarkdownTextChunker : ITextChunker
{
        ...
        ref bool firstChunkDone)
    {
        ...
        return GenerateChunksOptimized(text, fragments, maxChunk1Size, maxChunkNSize, separatorType, tokenizer, ref firstChunkDone);
    }
    ...
    internal static List<FragmentRange> SplitToFragmentsOptimized(string text, SeparatorTrie? separators)
    {
        if (separators is null)
        {
    ...
    private static List<string> MergeImageChunksOptimized(List<string> chunks)
    {
        if (chunks.Count <= 1)
        {
            return chunks;
    ...
    private static string NormalizeNewlinesOptimized(string input) => input.ReplaceLineEndings("\n");

    /// <summary>
    /// Range-based recursive split - returns ranges into original text instead of allocating strings.
    /// </summary>
        ...
        ref bool firstChunkDone)
    {
        ...
        // Get fragments for the working range - this still needs a substring for SplitToFragmentsOptimized
        // but the fragments returned use ranges relative to this substring
        var workingText = originalText[workingRange];
        var fragments = separatorType switch
        {
            SeparatorType.ExplicitSeparator => SplitToFragmentsOptimized(workingText, ExplicitSeparators),
            SeparatorType.PotentialSeparator => SplitToFragmentsOptimized(workingText, PotentialSeparators),
            SeparatorType.WeakSeparator1 => SplitToFragmentsOptimized(workingText, WeakSeparators1),
            SeparatorType.WeakSeparator2 => SplitToFragmentsOptimized(workingText, WeakSeparators2),
            SeparatorType.WeakSeparator3 => SplitToFragmentsOptimized(workingText, WeakSeparators3),
            SeparatorType.NotASeparator => SplitToFragmentsOptimized(workingText, null),
            _ => throw new ArgumentOutOfRangeException(nameof(separatorType), separatorType, null)
        };
    ...
    /// <summary>
    /// Range-based chunk generation - tracks indices instead of building strings.
    /// Returns ranges in original text coordinates.
    /// </summary>
